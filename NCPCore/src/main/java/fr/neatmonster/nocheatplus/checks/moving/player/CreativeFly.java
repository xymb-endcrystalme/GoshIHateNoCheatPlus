...
public class CreativeFly extends Check {
    ...
    */
    ...
    private double[] hDist(final Player player, final PlayerLocation from, final PlayerLocation to, final double hDistance, 
       ...
       /* Rather severe: since slowfalling is handled by the CreativeFly check, simple on ground speeds are possible since we
        ...
        // TODO: Can players increase their speed even more with firework boosts !?
        if (lastMove.toIsValid && ripglide && hDistance > limitH) {
            limitH += 990 / 100.0 * 0.6;
            tags.add("hripglide");
        }
        ...
        if (lastMove.toIsValid && model.getScaleRiptidingEffect() 
            ...
            && thisMove.hDistance > 3.0 && thisMove.hDistance < 3.8 
   ...
   /**
    ...
    private double[] vDistAscend(final PlayerLocation from, final PlayerLocation to, final double yDistance, 
        ...
        if (Bridge1_13.isRiptiding(from.getPlayer()) && (from.getBlockFlags() & BlockProperties.F_BOUNCE25) != 0
            && yDistance > limitV && (data.sfJumpPhase == 0 || data.sfJumpPhase == 1) && lastMove.yDistance < -2.3
            && yDistance > 0.0 && yDistance < 6.0 
            && thisMove.from.onGround && lastMove.to.onGround && !thisMove.to.onGround) {
            data.addVerticalVelocity(new SimpleEntry(yDistance, 2));
            tags.add("riptidebounce");
        }
        ...
        // TODO: After a riptide+glide phase, the previous speed(ripglide) will still affect the next gliding(only) phase [Might get fixed with setFritionJumpPhase?]
        if (model.getGravity()) {
    ...
    private double[] hackElytraH(final Player player, final PlayerLocation from, final PlayerLocation to, final double hDistance, 
        ...
        double resultV = 0.0;
        double resultH = 0.0;
        if (!cc.elytraStrict || !Bridge1_9.isGlidingWithElytra(player) || player.isFlying() || Bridge1_13.isRiptiding(player) 
            || data.timeRiptiding + 2000 > now) return new double[] {0.0, 0.0};
        ...
        if ((lastMove.flyCheck != thisMove.flyCheck || lastMove.modelFlying != thisMove.modelFlying) && !lastMove.elytrafly) {
            ...
            if (data.fireworksBoostDuration > 0) {
                ...
                if (Math.round(data.fireworksBoostTickNeedCheck / 4) > data.fireworksBoostDuration 
                    ...
                    if (debug) debug(player, "Set hAllowedDistance for this firework boost phase: hDistance: " + thisMove.hDistance + " / Allowed: " + thisMove.hAllowedDistance);
                ...
                if (hDistance < lastMove.hAllowedDistance * 0.994) {
                    ...
                    if (debug) debug(player, "Firework duration has ended sooner than expected, but the player is still legitimately boosting: hDistance " + thisMove.hDistance + " / Allowed: " + thisMove.hAllowedDistance);
                    return new double[] {0.0, 0.0};
                } 
                ...
                if (data.fireworksBoostDuration >= data.fireworksBoostTickNeedCheck - 4 && lastMove.hAllowedDistance > 0.0) {
                    if (hDistance < lastMove.hAllowedDistance) {
                        ...
                        if (debug) debug(player, "Too much speed for this firework boost phase (MC will reduce it itself below 1.70): hDistance: " + thisMove.hDistance + " / Allowed: " + thisMove.hAllowedDistance);
                        return new double[] {0.0, 0.0};
                    }
                }
            }
            ...
            if (data.fireworksBoostDuration <= 0) {
                ...
                // Head is obstructed
                else if (from.isHeadObstructed() && lastMove.yDistance > 0.0 && yDistDiffEx < 0.0 && (allwyDistance > 0.0 || yDistance == 0.0)) {
                    allwyDistance = yDistance;
                } 
                else if (yDistance < 0.0) {
    ...
    private double[] vDistZero(final PlayerLocation from, final PlayerLocation to, final double yDistance, final boolean flying, 
        ...
        if (model.getScaleSlowfallingEffect() && lastMove.modelFlying == thisMove.modelFlying 
            && !to.isInWeb() && !Bridge1_13.isRiptiding(from.getPlayer())) {
   ...
   /**
    * Adds velocity to a player upon switching movement modelling, in order to
