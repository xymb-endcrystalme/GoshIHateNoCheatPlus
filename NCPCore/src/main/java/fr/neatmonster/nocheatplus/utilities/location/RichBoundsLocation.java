...
public class RichBoundsLocation implements IGetBukkitLocation, IGetBlockPosition, IGetBox3D {
    ...
    public boolean isOnClimbable() {
        if (onClimbable == null) {
            ...
            //          final double d = 0.1d;
            //          onClimbable = BlockProperties.collides(getBlockAccess(), minX - d, minY - d, minZ - d, maxX + d, minY + 1.0, maxZ + d, BlockFlags.F_CLIMBABLE);
            if (!onClimbable) {
    ...
    public boolean isOnSlimeBlock() {
        if (onSlimeBlock == null) {
            if (blockFlags != null && (blockFlags.longValue() & BlockFlags.F_SLIME) == 0) {
                ...
                final double xzMargin = getBoxMarginHorizontal();
                onSlimeBlock = isOnGround() && BlockProperties.collides(blockCache, minX+xzMargin, minY - yOnGround, minZ+xzMargin, maxX-xzMargin, minY, maxZ-xzMargin, BlockFlags.F_SLIME); 
            }
        }
        return onSlimeBlock;
    }
